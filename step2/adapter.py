# Bước 2 (Cách 3) — Cut-Layer Adapter: Buộc hấp thụ qua 1 tầng Conv 1x1 tại cut layer
import torch
...
class Adapter(nn.Module):
    """
    Cut-Layer Adapter: 1 tầng Conv 1x1 (64 -> 64, không bias).
    ...
    Adapter buộc phải học phép biến đổi nghịch đảo:
        A ≈ P_pi^T
    giúp khôi phục chính xác 100% hoán vị kênh bí mật pi.
    """
    def __init__(self, channels=64):
        super().__init__()
        self.conv = nn.Conv2d(channels, channels, kernel_size=1, bias=False)
        # Khởi tạo Kaiming Normal

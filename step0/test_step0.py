...
def test_split_learning_step():
    ...
    # Chạy 1 epoch train
    loss = train_epoch(client, server, loader, opt_c, opt_s, criterion, device)
    print(f"[TEST] Train loss: {loss:.4f}")
    ...
    # Chạy evaluate
    acc = evaluate(client, server, loader, device)
    print(f"[TEST] Evaluate acc: {acc * 100:.2f}%")
